...
public class PayService {
    ...
    public PayResponse.DTO modifyPay(PayRequest.DTO reqDTO, SessionUser sessionUser, Integer payId) {
        Pay pay = payRepository.findById(payId)
                .orElseThrow(() -> new ApiException404("존재 하지 않는 결제 정보입니다"));

        // 결제 권한 체크
        if (!sessionUser.getId().equals(pay.getReservation().getUser().getId())) {
            throw new ApiException404("결제를 진행할 권한이 없습니다");
        }
    ...
    public PayResponse.RefundAndCancelReservationDTO refundAndCancel(Integer payId, SessionUser sessionUser) {
        Pay pay = payRepository.findById(payId)
                .orElseThrow(() -> new ApiException404("존재하지 않는 결제 내역입니다"));
        ...
        if (pay.getReservation().getUser().getId() != sessionUser.getId()) {
            // 예약을 한 유저가 아닌 경우
            throw new ApiException404("예약을 취소할 권한이 없습니다");
        } else if (pay.getState() == REFUND) {
            // 이미 환불이 된 경우
            throw new ApiException404("이미 취소된 예약입니다");
        } else if (reservation.getCheckOutDate().isBefore(LocalDate.now())) {
            // 이미 다녀온 경우
            throw new ApiException404("이미 이용한 숙소입니다");
        }
